...
function M.spawn_action_windows(call_buffer, bookmarks, line_nr, col_nr, call_window, index)
	...
	local hide_handbook = config.getState("hide_buffer_handbook")

	if hide_handbook then
		if #bookmarks == 0 then
	...
	if hide_handbook and #bookmarks == 0 then
		...
		vim.keymap.set("n", indexes:sub(i, i), function()
			...
			if not found then
				if delete_mode then
					...
					if hide_handbook and #bookmarks == #to_delete then
						closeMenu(actions_buffer, call_buffer)
